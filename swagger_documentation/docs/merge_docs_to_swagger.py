...
class SwaggerDocsMerger:
    ...
    def _load_versions_config(self) -> Dict[str, Any]:
        ...
        if not config_file.exists():
            ...
            return {
                "default_version": "2.0",
                "versions": {
                    "2.0": {"deprecated": False, "description": "Current API version"}
                }
            }
                ...
                if exclusions_file.exists():
                    ...
                    if "exclude_paths" in exclusions_data:
                        ...
                        for pattern in exclusions_data["exclude_paths"]:
                            ...
                            # Find matching paths
                            matching_paths = [
                                path for path in merged["paths"].keys()
                                if regex.match(path)
                            ]
                    ...
                    if "exclude_schemas" in exclusions_data:
                        for schema_name in exclusions_data["exclude_schemas"]:
                            ...
                            if "*" in schema_name:
                                ...
                                matching_schemas = [
                                    name for name in merged["components"]["schemas"].keys()
                                    if regex.match(name)
                                ]
                    ...
                    if "exclude_security_schemes" in exclusions_data:
                        for scheme_name in exclusions_data["exclude_security_schemes"]:
                            ...
                            if "*" in scheme_name:
                                ...
                                matching_schemes = [
                                    name for name in merged["components"]["securitySchemes"].keys()
                                    if regex.match(name)
                                ]
    ...
    def _merge_component_type_versioned(self, merged: Dict[str, Any], version: str, 
                                       component_type: str, yaml_key: str, 
                                       exclusion_key: str):
                    ...
                    if exclusion_key in exclusions_data:
                        for item_name in exclusions_data[exclusion_key]:
                            ...
                            if "*" in item_name:
                                ...
                                # Find matching items
                                matching_items = [
                                    name for name in merged["components"][yaml_key].keys()
                                    if regex.match(name)
                                ]
    ...
    def _merge_request_bodies_versioned(self, merged: Dict[str, Any], version: str):
        ...
        self._merge_component_type_versioned(merged, version, "requestBodies", "requestBodies", "exclude_request_bodies")
